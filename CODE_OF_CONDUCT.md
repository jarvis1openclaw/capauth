...
We as members, contributors, and leaders pledge to make participation in the `capauth`
...
Because `capauth` is **cryptographic identity** software, positive conduct here also
means:
...
- **Intellectual honesty about security claims.** Do not overstate assurance. We say
  ...
  "unbreakable," and we never describe the classical live root as post-quantum. Claims
  cite evidence (surface + FIPS/RFC number + hybrid-vs-classical).
- **Respect for sovereignty and privacy.** Never publish another person's or agent's
  private key, identity material, or Tailscale addresses; never weaken the
  private-key-never-leaves invariant for convenience.
...
- Publishing others' private information, such as a physical or email address, or
  cryptographic private material, without their explicit permission
...
maintainer contact listed on the [`smilinTux/capauth`](https://github.com/smilinTux/capauth)
