...
| `src/capauth/crypto/` | Crypto backends: `pgpy_backend.py`, `gnupg_backend.py`, `sequoia_backend.py`. **Quantum-resistance:** identity is classical per surface (Ed25519/RSA, Shor-breakable; RFC 8032); hybrid sigs already shipped on per-message/DID surfaces (ML-DSA-65+Ed25519). Root PQC migration is **in progress, additive-first and reversible** — the live root identity (fp `02BC0EB3CAD31DB691A753C70C5629AB893F9746`) is **still classical** until the gated root-rotation ceremony with Chef's real key. GnuPG is signing-disqualified (its PQC support is ML-KEM encryption only), so `sequoia_backend.py` (sq CLI) is the only backend that can host a PQC signing root: target = primary ML-DSA-87+Ed448 (FIPS 204, NIST L5) + ML-KEM-1024+X448 encryption subkey (FIPS 203) on OpenPGP v6/RFC 9580, against pre-RFC draft-ietf-openpgp-pqc-17. See `docs/CRYPTO_SPEC.md` → Quantum-Resistance + master plan (skchat `docs/quantum-resistance-architecture.md`), epic `PQC-MIGRATION` (coord `e1d6ba2a`). |
| `src/capauth/crypto/sequoia_backend.py` | `SequoiaBackend` — PQC OpenPGP signing backend via the `sq` CLI subprocess (ML-DSA-87+Ed448 primary / ML-KEM-1024+X448 KEM, RFC 9580 v6); the **only** backend that signs post-quantum. |
