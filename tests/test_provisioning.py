...
def test_provision_makes_decide_allow_all_skchat_caps(tmp_path):
    base = tmp_path / "home"
    out = provision_subject("lumina@host", mode="verified", sign=True, base_dir=base)
...
def test_tofu_mode_provisioning_allows_only_inbox(tmp_path):
    ...
    base = tmp_path / "home"
    provision_subject("guest@host", mode="tofu", sign=True, base_dir=base)
...
def test_provision_returns_ids(tmp_path):
    out = provision_subject("op@host", sign=True, base_dir=tmp_path)
